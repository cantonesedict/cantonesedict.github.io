< /res/rules/boilerplate.cmdr
< /res/rules/syntax.cmdr
< /res/rules/links.cmdr

OrdinaryDictionaryReplacement: #.properties-override
- queue_position: AFTER #.boilerplate.properties-override
- apply_mode: SEQUENTIAL
* %title --> aa
* %date-modified --> 2023-07-21
* %copyright-prior-years -->

%%%

#{.williams} (A/) [[\J aa]]

--{.williams}
Words in _a_ [[\J a]] or _(a/)_ [[\J aa]],
are often heard beginning with _ng_ [[\J ng]],
as _ng(a/)_ [[\J ngaa]],
_ngai_ [[\J ngai]],
_ngat_ [[\J ngat]].
--


##{#1 .williams} (1)(A/) [[陰平; \J aa1]]

### 丫1 [[U+4E2B]] _(1)(a/)_ [[\J aa1]]

$$
W
  , [[(Page~1)]] <br>
    ==
    - A fork, a crotch;
    - the parting of two branches, fingers, or tines;
    - _(1)(a/) (4)t(')au_
      [[丫頭; \J aa1 tau4]]
      or
      _(1)(a/) (4)w(a/)n_
      [[丫鬟; \J aa1 waan4]],
      a slave girl;
    - _(1)(a/) kok(8) kai(3)_
      [[丫角髻; \J aa1 gok3 gai3]]
      a little boy;
    - _(1)(a/) (1)ch(')(a/)_
      [[丫杈; \J aa1 caa1]],
      a fork, a rest for clothes' sticks;
    - _sh(u")(6) (1)(a/)_
      [[樹丫; \J syu6 aa1]]
      fork of a tree;
    - _(1)s(a/)m (1)(a/) l(o\)(6)_
      [[三丫路; \J saam1 aa1 lou6]]
      meeting of three ways, a trivium.
    ==
C
  ,
    =={.modern}
    - 丫頭 & 丫鬟, slave girl: it's 1856.
    ==
$$

### 吖1 [[U+5416]] _(1)(a/)_ [[\J aa1]]

$$
A
  , [[$呀1; $啊1]]
V
  , [[\J a1: shorter syllable]]
W
  ,
    [[(Page~1)]] <br>
    A colloquial word.
    ==
    - An interrogative particle;
    - also one indicating that the affirmation is indisputable;
    - a mere final answering tone, having no meaning;
    - _(2)h(o\) (1)(a/)_
        [[好吖; \J hou2 aa1]]
        well, very well;
    - _(1)(a/) wai(3)_
        [[?; \J aa1 wai3]]
        poor, necessitous;
    - _(5)k(')(u") (2)k(o\)m w(a/)(6) (1)(a/)_
        [[佢噉話吖; \J keoi5 gam2 waa6 aa1]],
        he said so;
    - _mat(7) (5)y(e/) (1)(a/)_
        [[乜嘢吖; \J mat1 je5 aa1]],
        what?
    - _(2)k(o\)m ts(o\)(6) (a/)_
        [[噉做吖; \J gam2 zou6 aa1]],
        no, this is the way to do it.
    ==
C
  ,
    =={.modern}
    - [Jyutping suggests 吖 as the canonical character for aa1.][jpp]
    ==
$$

### 呀1 [[U+5440]] _(1)(a/)_ [[\J aa1]]

$$
A
  , [[$吖1; $啊1]]
V
  , [[\J a1: shorter syllable]]
W
  ,
    [[(Page~1)]] <br>
    A colloquial word.
    ==
    - An interjection used in answers, denoting surprise or alarm;
    - alas! dreadful.
    ==
    Read _(4)ng(a/)_ [[\J ngaa4]]: [[see [`ngaa#呀4`]]]. <## TODO ##>
C
  ,
    =={.modern}
    - Williams does not provide a separate entry
      under _(4)ng(a/)_ [[\J ngaa4]].
    ==
S
  ,
    =={.modern}
    - [`aa#呀3`]
    - [`aa#呀4`]
    - [`aa#呀6`]
    - [`ngaa#呀4`]
    ==
$$

###+ 啊1 [[U+554A]] _(1)(a/)_ [[\J aa1]]

$$
A
  , [[$吖1; $呀1]]
V
  , [[\J a1: shorter syllable]]
W
  ,
    [[Not present]]
C
  ,
    =={.modern}
    - An unpopular alternative for $吖1 or $呀1.
    ==
S
  ,
    =={.modern}
    - [`aa#啊3`]
    ==
$$

### 鴉1 [[U+9D09]] _(1)(a/)_ [[\J aa1]]

$$
W
  ,
    ==
    - A raven with a white streak on its breast;
    - _(5)l(o\) (1)(a/)_
      [[老鴉; \J lou5 aa1]],
      a raven or crow;
    - _(4)t(')(o\) (1)(a/)_
      [[塗鴉; \J tou4 aa1]],
      written roughly;
    - _(1)(a/) p(')(i/)n(3) (4)nai_
      [[鴉片坭; \J aa1 pin3 nai4]],
      opium.
    ==
C
  ,
    =={.modern}
    - 塗鴉, written roughly: now graffiti.
    - 鴉片坭, opium: now just 鴉片.
    ==
$$


##{#2 .williams} (2)(A/) [[陰上; \J aa2]]

### 啞2 [[U+555E]] _(2)(a/)_ [[\J aa2]]

$$
A
  , $瘂2
W
  ,
    ==
    - The confused noise of children studying;
    - dumb;
    - to keep silent, for which the next [[$瘂2]] is used;
    - _(2)(a/) mai(6)_
      [[啞謎; \J aa2 mai6]]
      an enigma;
    - _(4)ts(')ing (4)n(a/)n (2)(a/) (2)yan_
      [[情難啞忍; \J cing4 naan4 aa2 yan2]],
      hard to bear it patiently.
    ==
    Also read _ak(7)_ [[\J ak1]]: [[see [`ak#啞1`]]]. <## TODO ##>
C
  ,
    =={.modern}
    - Williams does not provide a separate entry
      under _ak(7)_ [[\J ak1]].
    ==
S
  ,
    =={.modern}
    - [`ak#啞1`]
    ==
$$

### 瘂2 [[U+7602]] _(2)(a/)_ [[\J aa2]]

$$
A
  , $啞2
W
  ,
    ==
    - Dumb, unable to speak;
    - dull, faded;
    - a cracked sound, as a bell;
    - wheezing;
    - nape of the neck;
    - _(2)(a/) (2)tsai_
      [[瘂仔; \J aa2 zai2]],
      a dumb boy;
    - _(2)(a/) (2)hau (4)m(o\) (4)(i/)n_
      [[瘂口無言; \J aa2 hau2 mou4 jin4]],
      will not speak, sulky;
    - _(2)(a/) tuk(9)_
      [[瘂毒; \J aa2 duk6]]
      dumb people are revengeful;
    - _(1)sheng t(a/)i(3) (2)(a/)_
      [[聲帶瘂; \J seng1 daai3 aa2]],
      the sound is indistinct.
    ==
$$


##{#3 .williams} (A/)(3) [[陰去; \J aa3]]

### 亞3 [[U+4E9E]] _(a/)(3)_ [[\J aa3]]

### 呀3 [[U+5440]] _(a/)(3)_ [[\J aa3]]

$$
S
  ,
    =={.modern}
    - [`aa#呀1`]
    - [`aa#呀4`]
    - [`aa#呀6`]
    - [`ngaa#呀4`]
    ==
$$

###+ 啊3 [[U+554A]] _(a/)(3)_ [[\J aa3]]

$$
S
  ,
    =={.modern}
    - [`aa#啊1`]
    ==
$$

### 婭3 [[U+5A6D]] _(a/)(3)_ [[\J aa3]]

### 阿3 [[U+963F]] _(a/)(3)_ [[\J aa3]]


##{#4 .modern} (4)(A/) [[陽平; \J aa4]]

###+ 呀4 [[U+5440]] _(4)(a/)_ [[\J aa4]]

$$
S
  ,
    =={.modern}
    - [`aa#呀1`]
    - [`aa#呀3`]
    - [`aa#呀6`]
    - [`ngaa#呀4`]
    ==
$$


##{#6 .modern} (A/)(6) [[陽去; \J aa6]]

###+ 呀6 [[U+5440]] _(a/)(6)_ [[\J aa6]]

$$
S
  ,
    =={.modern}
    - [`aa#呀1`]
    - [`aa#呀3`]
    - [`aa#呀4`]
    - [`ngaa#呀4`]
    ==
$$
